...
def write_to_csv(output_file, row):
    with open(output_file, 'a', newline='') as file:
        ...
        # first repete by x, y, z for the same segment, then repeat for the next segment
                        + ['waypoints_{0}_seg_{1}'.format(j, i) for i in range(num_waypoints-1) for j in ['x', 'y', 'z']])
                        
        writer.writerow(row)
    return None

def plot_cumulative_tracking_error(sim_result_minsnap, sim_result_drag, sim_result_dragcomp, filename=None):
    # Cumulative Tracking Error for Minsnap
    error_minsnap = np.cumsum(np.linalg.norm(sim_result_minsnap['state']['x'] - sim_result_minsnap['flat']['x'], axis=1)**2)

    # Cumulative Tracking Error for Drag-aware
    error_drag = np.cumsum(np.linalg.norm(sim_result_drag['state']['x'] - sim_result_drag['flat']['x'], axis=1)**2)

    # Cumulative Tracking Error for Drag with Compensation
    error_dragcomp = np.cumsum(np.linalg.norm(sim_result_dragcomp['state']['x'] - sim_result_dragcomp['flat']['x'], axis=1)**2)

    plt.figure()
    plt.plot(sim_result_minsnap['time'], error_minsnap, label='Minsnap', linewidth=4, color='red')
    plt.plot(sim_result_drag['time'], error_drag, label='Drag-aware', linewidth=4, color='blue')
    plt.plot(sim_result_dragcomp['time'], error_dragcomp, label='Minsnap with Drag Compensation', linewidth=4, color='green')
...
def plot_results_with_drag(sim_result_minsnap, sim_result_nn, sim_result_drag, waypoints, initial_cost, predicted_cost, drag_comp_cost, filename=None, waypoints_time=None):
    ...
    ax_traj.plot3D(sim_result_minsnap["flat"]["x"][:, 0], sim_result_minsnap["flat"]["x"][:, 1], sim_result_minsnap["flat"]["x"][:, 2], color='lightcoral', linestyle='--', linewidth=4, label='Minsnap Reference')
    ax_traj.plot3D(sim_result_minsnap["state"]["x"][:, 0], sim_result_minsnap["state"]["x"][:, 1], sim_result_minsnap["state"]["x"][:, 2], color='red', linestyle='-', linewidth=4, label='Minsnap Actual')
    ...
    ax_traj.plot3D(sim_result_drag["flat"]["x"][:, 0], sim_result_drag["flat"]["x"][:, 1], sim_result_drag["flat"]["x"][:, 2], color='lightgreen', linestyle='--', linewidth=4, label='minsnap+drag Reference')
    ax_traj.plot3D(sim_result_drag["state"]["x"][:, 0], sim_result_drag["state"]["x"][:, 1], sim_result_drag["state"]["x"][:, 2], color='green', linestyle='-', linewidth=4, label='minsnap+drag Actual')
    # # set x, y, z limits
    ...
    # Subplot for X
    ax_x.plot(sim_result_minsnap['time'], sim_result_minsnap['flat']['x'][:, 0], color='lightcoral', linestyle='--', linewidth=4)
    ax_x.plot(sim_result_minsnap['time'], sim_result_minsnap['state']['x'][:, 0], color='red', linestyle='-', linewidth=4)
    ax_x.plot(sim_result_nn['time'], sim_result_nn['flat']['x'][:, 0], color='lightskyblue', linestyle='--', linewidth=4)
    ax_x.plot(sim_result_nn['time'], sim_result_nn['state']['x'][:, 0], color='blue', linestyle='-', linewidth=4)
    ax_x.plot(sim_result_drag['time'], sim_result_drag['flat']['x'][:, 0], color='lightgreen', linestyle='--', linewidth=4)
    ax_x.plot(sim_result_drag['time'], sim_result_drag['state']['x'][:, 0], color='green', linestyle='-', linewidth=4)
    ...
    # Subplot for Y
    ax_y.plot(sim_result_minsnap['time'], sim_result_minsnap['flat']['x'][:, 1], color='lightcoral', linestyle='--', linewidth=4)
    ax_y.plot(sim_result_minsnap['time'], sim_result_minsnap['state']['x'][:, 1], color='red', linestyle='-', linewidth=4)
    ax_y.plot(sim_result_nn['time'], sim_result_nn['flat']['x'][:, 1], color='lightskyblue', linestyle='--', linewidth=4)
    ax_y.plot(sim_result_nn['time'], sim_result_nn['state']['x'][:, 1], color='blue', linestyle='-', linewidth=4)
    ax_y.plot(sim_result_drag['time'], sim_result_drag['flat']['x'][:, 1], color='lightgreen', linestyle='--', linewidth=4)
    ax_y.plot(sim_result_drag['time'], sim_result_drag['state']['x'][:, 1], color='green', linestyle='-', linewidth=4)
    ...
    # Subplot for Z
    ax_z.plot(sim_result_minsnap['time'], sim_result_minsnap['flat']['x'][:, 2], color='lightcoral', linestyle='--', linewidth=4)
    ax_z.plot(sim_result_minsnap['time'], sim_result_minsnap['state']['x'][:, 2], color='red', linestyle='-', linewidth=4)
    ax_z.plot(sim_result_nn['time'], sim_result_nn['flat']['x'][:, 2], color='lightskyblue', linestyle='--', linewidth=4)
    ax_z.plot(sim_result_nn['time'], sim_result_nn['state']['x'][:, 2], color='blue', linestyle='-', linewidth=4)
    ax_z.plot(sim_result_drag['time'], sim_result_drag['flat']['x'][:, 2], color='lightgreen', linestyle='--', linewidth=4)
    ax_z.plot(sim_result_drag['time'], sim_result_drag['state']['x'][:, 2], color='green', linestyle='-', linewidth=4)
    ...
    for ax, dim in zip([ax_x, ax_y, ax_z], [0, 1, 2]):
        ax.scatter(waypoints_time, waypoints[:, dim], c='k', marker='o', s=60, label='Waypoints')

    # Subplot for Yaw
    ax_yaw.plot(sim_result_minsnap['time'], actual_yaw_init, color='red', linestyle='-', linewidth=4)
    ax_yaw.plot(sim_result_minsnap['time'], sim_result_minsnap['flat']['yaw'], color='lightcoral', linestyle='--', linewidth=4)
    ax_yaw.plot(sim_result_nn['time'], actual_yaw_nn, color='blue', linestyle='-', linewidth=4)
    ax_yaw.plot(sim_result_nn['time'], sim_result_nn['flat']['yaw'], color='lightskyblue', linestyle='--', linewidth=4)
    ax_yaw.plot(sim_result_drag['time'], actual_yaw_drag, color='green', linestyle='-', linewidth=4)
    ax_yaw.plot(sim_result_drag['time'], sim_result_drag['flat']['yaw'], color='lightgreen', linestyle='--', linewidth=4)
    ...
    ax_yaw.scatter(waypoints_time, np.zeros(len(waypoints)), c='k', marker='o', s=60, label='Waypoints')
...
def plot_results_only_for_drag(sim_result_drag,sim_result_nn, waypoints, drag_comp_cost, predicted_cost, filename=None, waypoints_time=None):
    actual_yaw_drag = compute_yaw_from_quaternion(sim_result_drag['state']['q'])
    actual_yaw_nn = compute_yaw_from_quaternion(sim_result_nn['state']['q'])
...
def plot_results(sim_result_init, sim_result_nn, waypoints, initial_cost, predicted_cost, filename=None, waypoints_time=None):
    ...
    ax_traj = fig.add_subplot(121, projection="3d")
    # Minsnap trajectories
    ax_traj.plot3D(sim_result_init["flat"]["x"][:, 0], sim_result_init["flat"]["x"][:, 1], sim_result_init["flat"]["x"][:, 2], color='lightcoral', linestyle='--', linewidth=4, label='Minsnap Reference')
    ax_traj.plot3D(sim_result_init["state"]["x"][:, 0], sim_result_init["state"]["x"][:, 1], sim_result_init["state"]["x"][:, 2], color='red', linestyle='-', linewidth=4, label='Minsnap Actual')
    # Our method trajectories
    ax_traj.plot3D(sim_result_nn["flat"]["x"][:, 0], sim_result_nn["flat"]["x"][:, 1], sim_result_nn["flat"]["x"][:, 2], color='lightskyblue', linestyle='--', linewidth=4, label='Drag-aware Reference')
    ax_traj.plot3D(sim_result_nn["state"]["x"][:, 0], sim_result_nn["state"]["x"][:, 1], sim_result_nn["state"]["x"][:, 2], color='blue', linestyle='-', linewidth=4, label='Drag-aware Actual')
    # # set x, y, z limits
    ...
    # Subplot for X
    ax_x.plot(sim_result_init['time'], sim_result_init['flat']['x'][:, 0], color='lightcoral', linestyle='--', linewidth=4)
    ax_x.plot(sim_result_init['time'], sim_result_init['state']['x'][:, 0], color='red', linestyle='-', linewidth=4)
    ax_x.plot(sim_result_nn['time'], sim_result_nn['flat']['x'][:, 0], color='lightskyblue', linestyle='--', linewidth=4)
    ax_x.plot(sim_result_nn['time'], sim_result_nn['state']['x'][:, 0], color='blue', linestyle='-', linewidth=4)
    ax_x.set_title('X Position Over Time', fontsize=20)
    ax_x.set_xlabel('Time(s)', fontsize=18)
    ...
    # Subplot for Y
    ax_y.plot(sim_result_init['time'], sim_result_init['flat']['x'][:, 1], color='lightcoral', linestyle='--', linewidth=4)
    ax_y.plot(sim_result_init['time'], sim_result_init['state']['x'][:, 1], color='red', linestyle='-', linewidth=4)
    ax_y.plot(sim_result_nn['time'], sim_result_nn['flat']['x'][:, 1], color='lightskyblue', linestyle='--', linewidth=4)
    ax_y.plot(sim_result_nn['time'], sim_result_nn['state']['x'][:, 1], color='blue', linestyle='-', linewidth=4)
    ax_y.set_title('Y Position Over Time', fontsize=20)
    ax_y.set_xlabel('Time(s)', fontsize=18)
    ...
    # Subplot for Z
    ax_z.plot(sim_result_init['time'], sim_result_init['flat']['x'][:, 2], color='lightcoral', linestyle='--', linewidth=4)
    ax_z.plot(sim_result_init['time'], sim_result_init['state']['x'][:, 2], color='red', linestyle='-', linewidth=4)
    ax_z.plot(sim_result_nn['time'], sim_result_nn['flat']['x'][:, 2], color='lightskyblue', linestyle='--', linewidth=4)
    ax_z.plot(sim_result_nn['time'], sim_result_nn['state']['x'][:, 2], color='blue', linestyle='-', linewidth=4)
    ax_z.set_title('Z Position Over Time', fontsize=20)
    ax_z.set_xlabel('Time(s)', fontsize=18)
    ...
    # Subplot for Yaw
    ax_yaw.plot(sim_result_init['time'], actual_yaw_init, color='red', linestyle='-', linewidth=4)
    ax_yaw.plot(sim_result_init['time'], sim_result_init['flat']['yaw'], color='lightcoral', linestyle='--', linewidth=4)
    ax_yaw.plot(sim_result_nn['time'], actual_yaw_nn, color='blue', linestyle='-', linewidth=4)
    ax_yaw.plot(sim_result_nn['time'], sim_result_nn['flat']['yaw'], color='lightskyblue', linestyle='--', linewidth=4)
    ax_yaw.set_title('Yaw Angle Over Time', fontsize=20)
    ax_yaw.set_xlabel('Time(s)', fontsize=18)
...
def main():
    ...
    sim_result_nn, trajectory_cost_nn,_,nan_encountered, summary_result_nn = run_simulation_and_compute_cost(waypoints, yaw_angles_zero, vavg, use_neural_network=True, regularizer=vf, vehicle=vehicle, controller=controller, robust_c=rho)
