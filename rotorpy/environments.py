...
class Environment:
    ...
    def __init__(
        self,
        vehicle,  # vehicle object, must be specified.
        ...
        if world is None:
            ...
            self.world = World.empty(
                (-wbound, wbound, -wbound, wbound, -wbound, wbound)
            )
        else:
            self.world = world
        ...
        if mocap is None:
            ...
            mocap_params = {
                "pos_noise_density": 0.0005
                * np.ones((3,)),  # noise density for position
                "vel_noise_density": 0.0010
                * np.ones((3,)),  # noise density for velocity
                "att_noise_density": 0.0005
                * np.ones((3,)),  # noise density for attitude
                "rate_noise_density": 0.0005
                * np.ones((3,)),  # noise density for body rates
            ...
            self.mocap = MotionCapture(
                sampling_rate=sim_rate, mocap_params=mocap_params, with_artifacts=False
            )
        else:
            self.mocap = mocap
    ...
    def run(
        self,
        t_final=10,  # The maximum duration of the environment in seconds
        ...
        if verbose:
            ...
            print("-------------------RESULTS-----------------------")
            print(
                "SIM TIME -- %3.2f seconds | WALL TIME -- %3.2f seconds"
        ...
        if animate_bool:
            # Do animation here
            visualizer.animate_results(waypoints=waypoints, fname=fname, animate_wind=animate_wind)
        if plot:
            # Do plotting here
            visualizer.plot_results(
                plot_mocap=plot_mocap, plot_estimator=plot_estimator, plot_imu=plot_imu
            )
            if not animate_bool:
                plt.show()
    ...
    def save_to_csv(self, fname):
        ...
        if self.result is None:
            print(
                "Error: cannot save if no results have been generated! Aborting save."
            )
            return
        else:
            if not ".csv" in fname:
