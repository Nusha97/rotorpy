...
if __name__ == "__main__":
    ...
    traj = MinSnap(
        waypoints, yaw_angles, v_max=5, v_avg=v_avg, v_start=v_start, v_end=v_end
    )
    ...
    (fig, axes) = plt.subplots(
        nrows=5, ncols=1, sharex=True, num="Translational Flat Outputs"
    )
    ax = axes[0]
    ...
    ax = axes
    ax.scatter(
        x[:, 0], x[:, 1], c=cm.winter(speed / speed.max()), s=4, label="Flat Output"
    )
