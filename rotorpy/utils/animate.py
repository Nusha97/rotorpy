...
def animate(
    ...
    traj_line, = ax.plot([], [], [], 'b-', label='Trajectory')  # Initialize the trajectory line
    ...
    def update(frame):
        ...
        # ax.plot(*np.vstack([waypoints[0], position[:frame+1, :]]).T, color='blue')
    ...
    if filename is not None:
        ...
        ani.save(path, writer="ffmpeg", fps=render_fps, dpi=100)
        if close_on_finish:
            plt.close(fig)
            ani = None
